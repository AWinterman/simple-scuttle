...
function Ring(n, config) {
  ...
  for(var i = 0; i < n; ++i) {
    this.nodes[i] = {
      gossip: new gossip.Gossip(id(i), config)
    }
  }
...
function Random(n, config) {
  ...
  while(this.nodes.length < n) {
    i++
    this.nodes.push({gossip: new gossip.Gossip(id(i), config)})
  }
...
function PairConflict(config) {
  ...
  var A = {gossip: new gossip.Gossip('A', config)}
    , B = {gossip: new gossip.Gossip('A', config)}
...
function Pair(config) {
  ...
  var A = {gossip: new gossip.Gossip('A', config)}
    , B = {gossip: new gossip.Gossip('B', config)}
