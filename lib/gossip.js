...
function Gossip(id, mtu, max_history, overwrite) {
  if(!(this instanceof Gossip)) {
    return new Gossip(id, mtu, max_history, overwrite)
  }
  ...
  this.overwrite = overwrite || order
...
proto._transform = function(data, encoding, callback) {
  ...
  if(!this.overwrite(current, data)) {
    return callback()
  }
